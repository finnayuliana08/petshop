	...
	<div class="container-fluid">
		...
		<div class="row">
			<div class="col-md-12">
				<!-- TABLE STRIPED -->
				<div class="panel">
					<div class="panel-heading">CARI HEWAN</div>
					<div class="panel-body">
						<form action="<?php echo base_url('index.php/Transaksi/cari_hewan') ?>" method="post">
							<div class="row">
								<div class="col-md-9">
									<input type="text" class="form-control input-lg" placeholder="Kode Hewan" name="kode_hewan" required>
								</div>
								<div class="col-md-3">
								...
								<?php
									...
									if($cart_transaksi != NULL){
										foreach ($cart_transaksi as $cart) {
											...
											echo '
												<tr>
													<input type="hidden" name="id_hewan[]" value="'.$cart->id_hewan.'">
													<td>'.$no.'</td>
                          <td>'.$cart->kode_hewan.'</td>
													<td>'.$cart->nama_hewan.'</td>
													<td>'.$cart->jenis_hewan.'</td>
													<td>'.$cart->jenis_kelamin.'</td>
                          <td>'.$cart->stok.'</td>
                          <td>'.$cart->harga.'</td>
													<td>
														<input type="number" name="jumlah[]" class="form-control" onchange="hitung_subtotal
                            ('.$cart->id.','.$cart->harga.',this.value,'.$cart->id.')" value="'.$cart->jumlah.'">
													</td>
...
<script type="text/javascript">
	...
	function hitung_subtotal(id,harga,qty,id_hewan)
	{
		var price;
		price = harga*qty;
		...
		$.post("<?php echo base_url('index.php/transaksi/ubah_jumlah_cart') ?>",
	    {
	    	...
	    	console.log(data);
	    	if(data == '0'){
	    		alert("Stok Hewan tidak mencukupi!");
	    	}
	    });
